...
function: Scroller.setAll
    Search for divs of the class 'oferte_new_slider' and wrap them in a Scroller.
 */
Scroller.setAll = function () {
    $$('.oferte_new_slider').each(function(item) {
...
function: Scroller.reset
    If the passed element has class 'oferte_new_slider', wrap it in a Scroller.
 */
Scroller.reset = function (body_id) {
    if ($(body_id).className.match(new RegExp("(^|\\s)oferte_new_slider(\\s|$)"))) {
